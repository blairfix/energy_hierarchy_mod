...
int main()
{
    ...
    arma::mat best_model(n_iteration_final,2);     // best fit model
    ...
    // model results vectors
    arma::vec mod_management_fraction(n_iteration_final);
    arma::vec mod_energy_pc(n_iteration_final);


    // loop over alpha vector
    boost::progress_display progress_final(n_iteration_final);
    #pragma omp parallel for firstprivate(n_firms, best_span, alpha_vec_final)

    for(int alpha_iteration = 0; alpha_iteration < n_iteration_final; alpha_iteration++){

        // size distribution of firms
        arma::uvec  firm_vec = rpld(n_firms, 1, alpha_vec_final[alpha_iteration], 2300000,  true); // power law firm size distribution
